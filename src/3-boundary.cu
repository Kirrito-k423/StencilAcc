...
__global__ void stencil(int row_num, int col_num, int *arr_data, int *result) {
    if (blockIdx.x==0||blockIdx.x==511||blockIdx.y==0||blockIdx.y==511){
        ...
        auto data0 = arr_data[index];
        // up
        auto data1 = arr_data[(current_row - 1) * col_num + current_col];
        // down
        auto data2 = arr_data[(current_row + 1) * col_num + current_col];
        // left
        auto data3 = arr_data[current_row * col_num + (current_col - 1) ];
        // right
        auto data4 = arr_data[current_row * col_num + (current_col + 1) ];
