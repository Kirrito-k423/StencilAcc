...
__global__ void stencil(int row_num, int col_num, int *arr_data, int *result) {
    ...
    if (blockIdx.x==0||blockIdx.x==511||blockIdx.y==0||blockIdx.y==511){
        ...
        // up left part
        sdata[local_index - local_col_num - 1] = arr_data[(global_row - 1) * col_num + global_col - 1];
        // down right part
        sdata[local_index + local_col_num + 1] = arr_data[(global_row + 1) * col_num + global_col + 1];
        // up right corner
        if(local_row==1){
            if(local_col==1)
            sdata[local_index-1] = arr_data[global_row * col_num + (global_col - 1) ]; // 要改变三个点，但是我感觉写一个就超时了。
            // right
            if(local_col==2)
